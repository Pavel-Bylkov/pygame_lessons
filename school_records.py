...
возвожность добавления ученика, предметов и оценок.
...
while run:
    ...
    if choice == "0":
        ...
        print_records(base, list_classes)
    elif choice == "2":
        print_classes(list_classes)
    elif choice == "3":
        add_class(base, list_classes, FILE_NAME)
    elif choice == "4":
        print_users(base)
    elif choice == "5":
        add_user(base, list_classes, FILE_NAME)
    elif choice == "6":
        add_or_edit_grade(base, list_classes, FILE_NAME)
    elif choice == "7":
        del_class(base, list_classes, FILE_NAME)
    elif choice == "8":
        del_grade(base, list_classes, FILE_NAME)
    elif choice == "9":
        del_user(base, list_classes, FILE_NAME)
    else:
        print("Выбор не распознан. Повторите ввод!")
    time.sleep(2)
end_program()