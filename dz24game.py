"""В анимационной игре добавить световой слой и луч освещения на
основного персонажа, который включается/выключается клавишей."""
...
"""Сделать цвет луча рандомным каждый раз при нажатии."""
...
def ex2():
    ...
    class Coin(arcade.AnimatedTimeBasedSprite):
        def __init__(self, center_x, center_y, scale=0.2):
            ...
            self.center_x = center_x
            self.center_y = center_y
            self.light = Light(center_x=self.center_x,
                               center_y=self.center_y,
                               radius=30, color=arcade.color.WHITE,
                               mode="soft")
    ...
    class MyGame(MyView):
        def __init__(self):
            ...
            for i in range(20):
                coin = Coin(center_x=random.randint(20, 780),
                            center_y=random.randint(20, 580))
                self.coin_list.append(coin)
                self.light_layer.add(coin.light)
        ...
        def on_update(self, delta_time):
            ...
            for i in collision:
                self.sound_collect.play(self.window.volume)
                i.set_position(random.randint(20, 780), random.randint(20, 580))
                self.score += 1
            self.player_light.position = self.player.position
        ...
        def on_key_release(self, symbol, modifiers):
            if symbol in (arcade.key.D, arcade.key.A, arcade.key.LEFT, arcade.key.RIGHT):
                self.player.change_x = 0
